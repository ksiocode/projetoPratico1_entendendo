...
void ColetaDadosVetorStruct(Player *players_ref)  //Passa o tipo jogador mas não é usado é só para sobrecarga da função
{
	for (int i = 0; i < MAX_PLAYERS; i++)
	{
		cout << "Digite o Nome Completo do Usuario: ";
		cin.ignore(); // Remove a Quebra de Linha do Buffer para o Getline
		cin.getline(players_ref[i].s_colectName, 64); //Pega o conteudo da linha inteira com Espacos e guarda em colectName
		if (players_ref[i].s_colectName[0] == '0' || strcmp(players[i].s_colectName, "sair") == 0)
		{
			break; //Saida caso digite 0 no nome, ou "sair" no vetor
		}
		cout << "Digite o Login do Usuario: ";
		cin >> players_ref[i].s_colectLogin;
		cout << "Digite um Senha para o Usuario: ";
		cin >> players_ref[i].s_colectSenha;
		cout << "Digite a Idade: ";
		cin >> players_ref[i].s_colectIdade;
		cout << "Digite o Sexo \'M' ou \'F': ";
		cin >> players_ref[i].s_colectSexo;
		cout << "Digite o Estado UF: ";
		cin >> players_ref[i].s_colectEstado;
		cout << endl;
	}
	cout << endl;
}
...
PlayerObj player1Obj; //Percebe que funciona igualmente a struct pois a class(Objeto) age igual a Struct com a diferença que a class por padrão vem Private no Modificador de Acesso, ai eu setei public:

void ColetaDados(PlayerObj *jogador)  //Passa o tipo jogador mas não é usado é só para sobrecarga da função
{
	cout << "Digite o Nome Completo do Usuario: ";
	cin.ignore(); // Remove a Quebra de Linha do Buffer para o Getline
	cin.getline(jogador->s_colectName, 64); //Pega o conteudo da linha inteira com Espacos e guarda em colectName
